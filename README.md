...
3. 查看生成、跳过数量及结构化 warning；基础草稿会自动进入“候选用例审核工作台”。
4. 在工作台逐条勾选，并直接编辑名称、方法、相对路径、查询参数、JSON 请求体和预期状态码。
5. 使用“替换测试编辑器”，或在被测地址一致且用例 ID 不冲突时使用“追加到测试编辑器”。
6. 在运行配置中做最终检查，确认后再手动点击“运行测试”。
...
4. AI 候选会与 OpenAPI 基础草稿汇总到审核工作台，并显示类别和生成理由。
5. 逐条选择、编辑后再替换或追加到测试编辑器；工作台不允许编辑请求头、密钥、依赖或提取规则。
6. 只有再次手动点击“运行测试”才会访问被测服务；AI 不参与最终通过/失败判定。

审核工作台只保存在当前页面内存。候选来自不同被测地址时不能混入同一次运行；追加操作还会检查
当前编辑器与候选地址是否一致，以及用例 ID 是否冲突。JSON 输入错误会定位到具体候选并阻止载入，
但前端检查不替代后端 Pydantic 校验和目标 SSRF 策略。
...
本阶段主 Agent 已确认全量测试为 `197 passed`，分支覆盖率为 `91.48%`。后续代码变化后应重新执行
上述命令，并以当前工作区输出为准。
